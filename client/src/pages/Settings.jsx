  ...
  return (
    <div className="bg-gray-950 px-4 py-12 text-slate-100">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
        ...
        <form onSubmit={saveAccount} className="rounded-md border border-gray-800 bg-gray-900 p-6">
          <h2 className="text-xl font-bold text-white">Account Information</h2>
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-200" htmlFor="settings-name">
                Name
              </label>
              <input
                id="settings-name"
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-200" htmlFor="settings-email">
                Email
              </label>
              <input
                id="settings-email"
                ...
                readOnly
                className="w-full cursor-not-allowed rounded-md border border-gray-800 bg-gray-950 px-4 py-3 text-slate-400 outline-none"
              />
            </div>
            <div className="md:col-span-2">
              <label className="mb-2 block text-sm font-medium text-slate-200" htmlFor="settings-role">
                Current Role/Goal
              </label>
              <input
                id="settings-role"
          ...
          {accountMessage ? <p className="mt-4 text-sm text-emerald-300">{accountMessage}</p> : null}
          <button
            type="submit"
            disabled={accountSaving}
            className="mt-6 inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
          >
        ...
        <form onSubmit={savePreferences} className="rounded-md border border-gray-800 bg-gray-900 p-6">
          <h2 className="text-xl font-bold text-white">Preferences</h2>
          <div className="mt-6 grid gap-6 lg:grid-cols-3">
            <div>
              <p className="mb-3 text-sm font-medium text-slate-200">Interview Types</p>
              <div className="space-y-3">
                {interviewTypes.map((item) => (
                  <label key={item.value} className="flex items-center gap-3 text-sm text-slate-300">
                    <input
                      type="checkbox"
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-200" htmlFor="settings-difficulty">
                Difficulty
              </label>
              <select
                ...
                className="w-full rounded-md border border-gray-800 bg-gray-950 px-4 py-3 text-slate-100 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20"
              >
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-200" htmlFor="settings-topics">
                Preferred Topics
              </label>
              <input
                id="settings-topics"
          ...
          {preferencesMessage ? <p className="mt-4 text-sm text-emerald-300">{preferencesMessage}</p> : null}
          <button
            type="submit"
        ...
        <section className="rounded-md border border-gray-800 bg-gray-900 p-6">
          ...
          {!profile.has_password ? (
            <p className="mt-5 rounded-md border border-gray-800 bg-gray-950 p-4 text-sm text-slate-300">
              Password change is not available for Google sign-in accounts.
            </p>
          ) : (
            <div className="mt-5">
              ...
              {showPasswordForm ? (
                <form onSubmit={changePassword} className="mt-5 grid gap-5 md:grid-cols-3">
                  ...
                  <div className="md:col-span-3">
                    {passwordError ? <p className="mb-3 text-sm text-red-200">{passwordError}</p> : null}
                    <button
                      type="submit"
          ...
          {passwordMessage ? <p className="mt-4 text-sm text-emerald-300">{passwordMessage}</p> : null}
        </section>

        <section className="rounded-md border border-red-900/50 bg-gray-900 p-6">
